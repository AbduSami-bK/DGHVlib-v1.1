...
void clear_hw_table(__hw_table* hwtable) {
    for (unsigned long i = 1; i < hwtable->x; i++) {
        for (unsigned long j = 1; j < hwtable->y; j++) {
...
void clear_ev_table(__ev_table* evtable) {
    for (unsigned long i = 0; i < evtable->x; i++) {
        for (unsigned long j = 0; j < evtable->y; j++) {
...
unsigned long get_sc_ciphdivp_lsb(__cit* ciph, __sc_prikey* prikey, __sec_setting* para){
    ...
    unsigned long i, j, k, l, t, lsb, index, length;
    mpz_t res;
    __ev_table* ev_table;
                ...
                for(l = 0; l < length; l++){
                    if(t < para->theta){
                        if(prikey->s0[i][k] * prikey->s1[j][l] == 1){
                            set_ev_table(index, ciph->z[index], ev_table);
                            t++;
                        }
                    }
                    index++;
